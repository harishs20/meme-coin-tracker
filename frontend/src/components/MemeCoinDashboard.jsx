...
function LiveFeed({ messages = [] }) {
  if (messages.length === 0) {
    return <div style={{ fontSize: 11, color: "#555", padding: "10px 0", fontFamily: "'Rajdhani', sans-serif" }}>Listening for social signals...</div>;
  }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
      {messages.map((msg, i) => (
        <div key={msg.id} style={{
          padding: "6px 10px",
          background: "#ffffff08",
          border: `1px solid ${msg.sentiment === "positive" ? "#00FF8822" : msg.sentiment === "negative" ? "#FF004422" : "#ffffff22"}`,
          borderRadius: 6,
          fontSize: 10,
          color: "#ccc",
          fontFamily: "'Rajdhani', sans-serif",
          borderLeft: `3px solid ${msg.sentiment === "positive" ? "#00FF88" : msg.sentiment === "negative" ? "#FF0044" : "#888"}`,
...
function PredictionOracle({ coin, sentiment, velocity }) {
  ...
  const getPrediction = useCallback(async () => {
    ...
    try {
      ...
      setPrediction("⚡ Oracle offline. Backend server is not running!");
    } finally {
...
export default function MemeCoinDashboard() {
  ...
  useEffect(() => {
    socket.on('dashboard_update', (data) => {
      ...
      if (data.coins) {
        const lb = COINS.map(c => ({ 
          ...c, 
          score: data.coins[c.id]?.score || 0 
        ...
        <div style={{ display: "grid", gridTemplateColumns: "280px 1fr 260px", gap: 16 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div style={{ background: "#0A0A0F", border: "1px solid #ffffff0A", borderRadius: 12, padding: 16, minHeight: 180 }}>
              {activeTab === "live_feed" && <LiveFeed messages={selected.messages || []} />}
